...
#define MESH_DEBUG_LOG 1
	...
	/**** BEGIN MeshAsset class ****/
	HazelMeshAssetLegacy::HazelMeshAssetLegacy(const std::string& filename)
	{
	}

	HazelMeshAssetLegacy::HazelMeshAssetLegacy(const std::vector<VertexHazelLegacy>& vertices, const std::vector<IndexHazelLegacy>& indices, const glm::mat4& transform)
	{
	}

	HazelMeshAssetLegacy::~HazelMeshAssetLegacy()
	{
	}
	/**** END MeshAsset class ****/


	/**** BEGIN HazelMesh class ****/
	MeshHazelLegacy::MeshHazelLegacy(const std::string& filename)
	{
		m_FilePath = filename;
	...
	MeshHazelLegacy::MeshHazelLegacy(const std::string& filename, Ref<MoravaShader> shader, Ref<HazelMaterial> material, bool isAnimated)
	...
	void MeshHazelLegacy::Create()
	{
		...
		for (size_t m = 0; m < scene->mNumMeshes; m++)
		{
			...
			if (m_IsAnimated)
			{
				...
				{
					AnimatedVertexHazelLegacy vertex;
			...
			for (size_t i = 0; i < mesh->mNumFaces; i++)
			{
				...
				{
					Log::GetLogger()->error("MeshHazelLegacy: the face contains invalid number of indices (expected: 3, detected: {0})!", mesh->mFaces[i].mNumIndices);
					continue;
				}

				IndexHazelLegacy index = { mesh->mFaces[i].mIndices[0], mesh->mFaces[i].mIndices[1], mesh->mFaces[i].mIndices[2] };
		...
		if (scene->HasMaterials())
		{
			...
			for (uint32_t i = 0; i < scene->mNumMaterials; i++)
			{
				...
				/**** END to be removed from MeshHazelLegacy ****/
				...
				Ref<SubmeshHazelLegacy> submeshPtr = Ref<SubmeshHazelLegacy>();
				...
				if (hasAlbedoMap)
				{
					...
					if (texture && texture->Loaded())
					{
						...
						if (RendererAPI::Current() == RendererAPIType::Vulkan)
						{
							AddMaterialTextureWriteDescriptor(i, "u_AlbedoTexture", texture); // TODO: to be removed from MeshHazelLegacy
						}
						else
						{
				...
				if (fallback)
				{
					...
					if (RendererAPI::Current() == RendererAPIType::Vulkan)
					{
						AddMaterialTextureWriteDescriptor(i, "u_AlbedoTexture", whiteTexture); // TODO: to be removed from MeshHazelLegacy
					}
				}
				...
				if (hasNormalMap)
				{
					...
					if (texture && texture->Loaded())
					{
						...
						if (RendererAPI::Current() == RendererAPIType::Vulkan)
						{
							AddMaterialTextureWriteDescriptor(i, "u_NormalTexture", texture); // TODO: to be removed from MeshHazelLegacy
						}
						else
						{
				...
				if (fallback)
				{
					...
					if (RendererAPI::Current() == RendererAPIType::Vulkan)
					{
						AddMaterialTextureWriteDescriptor(i, "u_NormalTexture", whiteTexture); // TODO: to be removed from MeshHazelLegacy
					}
				}
				...
				if (aiMaterial->GetTexture(aiTextureType_SHININESS, 0, &aiTexPath) == AI_SUCCESS)
				{
					...
					if (texture->Loaded())
					{
						...
						if (RendererAPI::Current() == RendererAPIType::Vulkan)
						{
							AddMaterialTextureWriteDescriptor(i, "u_RoughnessTexture", texture); // TODO: to be removed from MeshHazelLegacy
						}
						else
						{
					...
					if (prop->mType == aiPTI_String)
					{
						...
						if (key == "$raw.ReflectionFactor|file")
						{
							...
							if (texture->Loaded())
							{
								...
								if (RendererAPI::Current() == RendererAPIType::Vulkan)
								{
									AddMaterialTextureWriteDescriptor(0, "u_MetalnessTexture", texture); // TODO: to be removed from MeshHazelLegacy
								}
								else
								{
									m_MeshShader->SetInt("u_MetalnessTexture", texture->GetID());
				...
				if (fallback)
				{
					...
					if (RendererAPI::Current() == RendererAPIType::Vulkan)
					{
						AddMaterialTextureWriteDescriptor(i, "u_MetalnessTexture", whiteTexture); // TODO: to be removed from MeshHazelLegacy
					}
				}
			}
			HZ_MESH_LOG("------------------------");
		...
		if (m_IsAnimated)
		{
			m_VertexBuffer = VertexBuffer::Create(m_AnimatedVertices.data(), (uint32_t)sizeof(AnimatedVertexHazelLegacy), (uint32_t)m_AnimatedVertices.size());
	...
	void MeshHazelLegacy::OnUpdate(Timestep ts, bool debug)
	{
		if (m_IsAnimated)
		{
	...
	void MeshHazelLegacy::TraverseNodes(aiNode* node, const glm::mat4& parentTransform, uint32_t level)
	{
	...
	uint32_t MeshHazelLegacy::FindPosition(float AnimationTime, const aiNodeAnim* pNodeAnim)
	{
	...
	uint32_t MeshHazelLegacy::FindRotation(float AnimationTime, const aiNodeAnim* pNodeAnim)
	{
	...
	uint32_t MeshHazelLegacy::FindScaling(float AnimationTime, const aiNodeAnim* pNodeAnim)
	{
	...
	glm::vec3 MeshHazelLegacy::InterpolateTranslation(float animationTime, const aiNodeAnim* nodeAnim)
	{
	...
	glm::quat MeshHazelLegacy::InterpolateRotation(float animationTime, const aiNodeAnim* nodeAnim)
	{
	...
	glm::vec3 MeshHazelLegacy::InterpolateScale(float animationTime, const aiNodeAnim* nodeAnim)
	{
	...
	void MeshHazelLegacy::SetupDefaultBaseMaterial()
	{
	...
	void MeshHazelLegacy::ImGuiNodeHierarchy(aiNode* node, const glm::mat4& parentTransform, uint32_t level)
	{
	...
	void MeshHazelLegacy::OnImGuiRender(uint32_t id, bool* p_open)
	{
		if (!m_Scene)
		{
	...
	void MeshHazelLegacy::ReadNodeHierarchy(float AnimationTime, const aiNode* pNode, const glm::mat4& parentTransform)
	{
	...
	const aiNodeAnim* MeshHazelLegacy::FindNodeAnim(const aiAnimation* animation, const std::string& nodeName)
	{
	...
	void MeshHazelLegacy::DumpVertexBuffer()
	{
	...
	const std::vector<TriangleHazelLegacy> MeshHazelLegacy::GetTriangleCache(uint32_t index) const
	{
		std::unordered_map<uint32_t, std::vector<TriangleHazelLegacy>>::const_iterator entry = m_TriangleCache.find(index);
		...
		return std::vector<TriangleHazelLegacy>();
	}

	void MeshHazelLegacy::Render(uint32_t samplerSlot, const glm::mat4& transform, const std::map<std::string, Ref<EnvMapMaterial>>& envMapMaterials)
	{
		...
		for (Ref<SubmeshHazelLegacy> submesh : m_Submeshes)
		{
			...
			Ref<MeshHazelLegacy> instance = this;
			std::string materialUUID = MaterialLibrary::GetSubmeshMaterialUUID(instance, submesh, Hazel::EntityHazelLegacy{});
	...
	void* MeshHazelLegacy::GetDescriptorSet()
	{
		return &s_DescriptorSet;
	}
	/**** END MeshHazelLegacy class ****/
