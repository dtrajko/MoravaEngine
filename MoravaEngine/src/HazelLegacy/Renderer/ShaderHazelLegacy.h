...
namespace HazelLegacy
{
	...
	class ShaderHazelLegacy : public Hazel::RefCounted
	{
	public:
		...
		static Hazel::Ref<ShaderHazelLegacy> Create(const std::string& filepath, bool forceCompile = false);
		static Hazel::Ref<ShaderHazelLegacy> CreateFromString(const std::string& source);

		virtual const std::unordered_map<std::string, ShaderBuffer>& GetShaderBuffers() const = 0;

		virtual const std::unordered_map<std::string, Hazel::ShaderResourceDeclaration>& GetResources() const = 0;

		virtual void AddShaderReloadedCallback(const ShaderReloadedCallback& callback) = 0;

		// Temporary, before we have an asset manager
		static std::vector<Hazel::Ref<ShaderHazelLegacy>> s_AllShaders;

		// Methods from Vulkan Week Day 1
		bool HasVSMaterialUniformBuffer();
		bool HasPSMaterialUniformBuffer();
		Hazel::Buffer GetVSMaterialUniformBuffer();
		Hazel::Buffer GetPSMaterialUniformBuffer();
	};

	// This should be eventually handled by the Asset Manager
	class ShaderLibraryHazelLegacy : public Hazel::RefCounted
	{
	public:
		ShaderLibraryHazelLegacy();
		~ShaderLibraryHazelLegacy();

		void Add(const Hazel::Ref<ShaderHazelLegacy>& shader);
		void Load(const std::string& path, bool forceCompile = false);
		void Load(const std::string& name, const std::string& path);

		Hazel::Ref<ShaderHazelLegacy> Get(const std::string& name);

	private:
		std::unordered_map<std::string, Hazel::Ref<ShaderHazelLegacy>> m_Shaders;
