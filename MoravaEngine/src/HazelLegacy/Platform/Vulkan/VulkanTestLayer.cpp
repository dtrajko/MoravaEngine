...
namespace Hazel {
	...
	struct SceneRendererData
	{
		...
		// SceneRendererOptions Options; // moved to VulkanRendererHazelLegacy
	};
	...
	void VulkanTestLayer::OnUpdate(Timestep ts)
	{
		...
		{
			VulkanRendererHazelLegacy::SubmitMeshTemp(mesh); // the method should be removed from VulkanRendererHazelLegacy
		}
	...
	{
		// VulkanRendererHazelLegacy::Draw(scene->GetCamera());
	}
	...
	/**** BEGIN moved to VulkanRendererHazelLegacy ****
	SceneRendererOptions& VulkanTestLayer::GetOptions()
	{
		return s_Data.Options;
	}
	/**** END moved to VulkanRendererHazelLegacy ****/
	...
	void VulkanTestLayer::GeometryPass(const glm::vec4& clearColor, const EditorCamera& camera)
	{
		...
		// VulkanRendererHazelLegacy::SubmitFullscreenQuadStatic(s_Data.SkyboxPipeline, s_Data.SkyboxMaterial);
		...
		// Grid
		if (VulkanRendererHazelLegacy::GetOptions().ShowGrid)
		{
			...
			// VulkanRendererHazelLegacy::RenderQuadStatic(s_Data.GridPipeline, s_Data.GridMaterial, transform);
		}

		if (VulkanRendererHazelLegacy::GetOptions().ShowBoundingBoxes)
		{
		...
		// HazelRenderer::Submit([=]() mutable {});
		{
			...
			for (auto& dc : s_Data.DrawList)
			{
				VulkanRendererHazelLegacy::RenderMeshStatic(dc.Mesh, dc.Transform);
			}
