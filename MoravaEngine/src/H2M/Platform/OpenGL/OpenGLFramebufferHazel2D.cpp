...
namespace H2M
{
	...
	OpenGLFramebufferHazel2D::OpenGLFramebufferHazel2D(const FramebufferSpecificationH2M& spec)
	...
	OpenGLFramebufferHazel2D::~OpenGLFramebufferHazel2D()
	{
	...
	void OpenGLFramebufferHazel2D::Invalidate()
	{
		if (m_RendererID)
		{
			glDeleteFramebuffers(1, &m_RendererID);
	...
	void OpenGLFramebufferHazel2D::Bind() const
	{
		glBindFramebuffer(GL_FRAMEBUFFER, m_RendererID);
		glViewport(0, 0, m_Specification.Width, m_Specification.Height);
	}

	void OpenGLFramebufferHazel2D::Unbind() const
	{
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
	}

	void OpenGLFramebufferHazel2D::Resize(uint32_t width, uint32_t height, bool forceRecreate)
	{
		if (width == 0 || height == 0 || width > s_MaxFramebufferSize || height > s_MaxFramebufferSize)
		{
			H2M_CORE_WARN("Attempted to rezize framebuffer to {0}, {1}", width, height);
			return;
		}
	...
	int OpenGLFramebufferHazel2D::ReadPixel(uint32_t attachmentIndex, int x, int y)
	{
	...
	void OpenGLFramebufferHazel2D::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		H2M_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size());
