...
namespace H2M {
	...
	/**** BEGIN moved to VulkanRendererH2M ****
	SceneRendererOptions& VulkanTestLayer::GetOptions()
	{
		return s_Data.Options;
	}
	/**** END moved to VulkanRendererH2M ****/
	...
	void VulkanTestLayer::GeometryPass(const glm::vec4& clearColor, const EditorCamera& camera)
	{
		...
		// VulkanRendererH2M::SubmitFullscreenQuadStatic(s_Data.SkyboxPipeline, s_Data.SkyboxMaterial);
		...
		// Grid
		if (VulkanRendererH2M::GetOptions().ShowGrid)
		{
			...
			// VulkanRendererH2M::RenderQuadStatic(s_Data.GridPipeline, s_Data.GridMaterial, transform);
		}

		if (VulkanRendererH2M::GetOptions().ShowBoundingBoxes)
		{
		...
		// HazelRenderer::Submit([=]() mutable {});
		{
			...
			for (auto& dc : s_Data.DrawList)
			{
				VulkanRendererH2M::RenderMeshStatic(dc.Mesh, dc.Transform);
			}
	...
	void VulkanTestLayer::UpdateImGuizmo()
	{
		if (VulkanTestLayer::s_ImGuizmoType != -1 && EntitySelection::s_SelectionContext.size())
		{
			...
			// Entity transform
			auto& transformComponent = selectedSubmesh.Entity.GetComponent<H2M::TransformComponent>();
