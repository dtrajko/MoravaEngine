...
namespace Hazel {
	...
	public:
		...
		virtual std::pair<Ref<TextureCubeH2M>, Ref<TextureCubeH2M>> CreateEnvironmentMap(const std::string& filepath) override;
		virtual Ref<TextureCubeH2M> CreatePreethamSky(float turbidity, float azimuth, float inclination) override;
		...
		static void RenderMeshVulkan(Ref<MeshH2M> mesh, VkCommandBuffer commandBuffer);
		...
		static void MapUniformBuffersVTL(Ref<MeshH2M> mesh, const EditorCamera& camera);
		/**** END methods moved from VulkanTestLayer to VulkanRenderer ****/

		static void SubmitMeshTemp(const Ref<MeshH2M>& mesh, const glm::mat4& transform = glm::mat4(1.0f)); // to be removed from VulkanRenderer
		...
		// Obsolete methods
		virtual void RenderMesh(Ref<Pipeline> pipeline, Ref<MeshH2M> mesh, const glm::mat4& transform) override;
		virtual void RenderMeshWithoutMaterial(Ref<Pipeline> pipeline, Ref<MeshH2M> mesh, const glm::mat4& transform) override;
		virtual void RenderQuad(Ref<Pipeline> pipeline, Ref<HazelMaterial> material, const glm::mat4& transform) override;

		void RenderMesh(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<UniformBufferSet> uniformBufferSet, Ref<StorageBufferSet> storageBufferSet, Ref<MeshH2M> mesh, Ref<MaterialTable> materialTable, const glm::mat4& transform);
		void RenderMeshWithMaterial(Ref<RenderCommandBuffer> renderCommandBuffer, Ref<Pipeline> pipeline, Ref<UniformBufferSet> uniformBufferSet, Ref<StorageBufferSet> storageBufferSet, Ref<MeshH2M> mesh, Ref<HazelMaterial> material, const glm::mat4& transform, Buffer additionalUniforms = Buffer());
