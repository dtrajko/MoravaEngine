...
VulkanSkyboxCube::VulkanSkyboxCube()
{
	...
	m_VertexBuffer = Hazel::VertexBuffer::Create(vertices.data(), (uint32_t)vertices.size() * sizeof(VertexSkyboxCube));
	m_IndexBuffer = Hazel::IndexBuffer::Create(indices.data(), (uint32_t)indices.size() * sizeof(uint32_t));
