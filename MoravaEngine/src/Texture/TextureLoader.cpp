...
H2M::Ref<MoravaTexture> TextureLoader::GetTexture(const char* fileLoc, bool flipVert, bool force)
{
	return GetTexture(fileLoc, flipVert, GL_LINEAR, force);
}

H2M::Ref<MoravaTexture> TextureLoader::GetTexture(const char* fileLoc, bool flipVert, GLenum filter, bool force)
{
	if (!force) {
		std::map<std::string, H2M::Ref<MoravaTexture>>::iterator it;
	...
	H2M::Ref<MoravaTexture> texture = MoravaTexture::Create(fileLoc, false, true, filter);
...
{
	std::map<std::string, H2M::Ref<MoravaTexture>>::reverse_iterator rit;
