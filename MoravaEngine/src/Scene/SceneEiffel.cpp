...
void SceneEiffel::UpdateImGui(float timestep, Window* mainWindow)
{
	...
	ImGui::Begin("Settings", &m_ShowWindowSettings);
	{
		...
		if (ImGui::CollapsingHeader("Display Info", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
		{
			...
			ImGui::DragFloat("", &sceneSettings.waterHeight, 0.1f, -2.0f, 20.0f, "%.2f");
			ImGui::Text("DirLight Direction");
			ImGui::SliderFloat3("", glm::value_ptr(m_LightDirection), -100.0f, 100.0f);
			ImGui::Text("DirLight Color");
			ImGui::ColorEdit3("", glm::value_ptr(m_LightColor));
	...
	// Log::GetLogger()->debug("m_ShowWindowSettings: '{0}'", m_ShowWindowSettings);
	// Log::GetLogger()->debug("m_ShowWindowFramebuffers: '{0}'", m_ShowWindowFramebuffers);
}
