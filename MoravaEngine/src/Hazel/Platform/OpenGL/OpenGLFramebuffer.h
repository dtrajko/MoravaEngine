...
namespace Hazel {
	...
	{
	public:
		OpenGLFramebuffer(const HazelFramebufferSpecification& spec);
		...
		virtual Ref<HazelImage2D> GetImage(uint32_t attachmentIndex = 0) const override { HZ_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size()); return m_ColorAttachments[attachmentIndex]; }
