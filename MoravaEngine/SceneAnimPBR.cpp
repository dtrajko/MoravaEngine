	...
	std::map<std::string, Shader*> shaders, std::map<std::string, int> uniforms)
{
    ...
    {
        m_MeshAnimPBR_M1911->Render(m_SamplerSlots["albedo"], m_Entities["M1911"].Transform.Transform, std::map<std::string, Ref<EnvMapMaterial>>());
    }

    if (m_Entities["AnimBoy"].Enabled)
    {
        m_MeshAnimPBR_AnimBoy->Render(m_SamplerSlots["albedo"], m_Entities["AnimBoy"].Transform.Transform, std::map<std::string, Ref<EnvMapMaterial>>());
    }
