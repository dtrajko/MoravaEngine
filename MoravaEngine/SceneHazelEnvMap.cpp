...
{
    if (!m_EnvironmentMap->m_IsViewportEnabled) return;
...
void SceneHazelEnvMap::UpdateImGui(float timestep, Window* mainWindow)
{
    ...
    m_EnvironmentMap->OnImGuiRender(mainWindow, m_RenderFramebuffer);
}
...
{
    m_EnvironmentMap->OnRender(mainWindow, m_RenderFramebuffer);
}
